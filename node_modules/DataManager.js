...
module.exports = {
    ...
    save: function (data, pFilePath, bFilePath) {
        ...
        bFilePath = bFilePath || path.normalize(
                path.join(
                    path.relative(process.cwd(), path.join(path.dirname(pFilePath), "backup/")),
                    path.basename(pFilePath, path.extname(pFilePath)) + ".bak.json"
                )
            );
    ...
    loadVideoData: function() {
        ...
        return this.load(primaryVideoData, backupVideoData)},
    loadCompicData: function() {
        console.log("Attempting to load video data from: " + primaryCompicData);
        return this.load(primaryCompicData, backupCompicData).items
    }
